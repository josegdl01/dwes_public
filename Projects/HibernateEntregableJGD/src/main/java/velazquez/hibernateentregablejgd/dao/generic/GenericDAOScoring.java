package velazquez.hibernateentregablejgd.dao.generic;
...
public class GenericDAOScoring<T> implements GenericDAO<T> {

    static final Logger log = LoggerFactory.getLogger(GenericDAOImpl.class);
    private Class<T> item;
    protected final Session session;

    public GenericDAOScoring(Session session) {
        setItem(
                (Class<T>)
    ...
    public void updateItem(T t) {
        ...
        if (Objects.isNull(session.find(this.item, t.getClass()))) {
            session.persist(t);
